...
function slice(n, target)
    ...
    df = CSV.write("data/complexity/slice32_rksg_n$(n).csv", DataFrame(name = Int[], origin_sc = Float64[], origin_tc = Float64[], origin_rwc = Float64[], sliced_sc = Float64[], sliced_tc = Float64[], sliced_rwc = Float64[], sliced_boundes = Int[]))
    for i in 1:10
        code = readjson(joinpath(@__DIR__, "..", "graphs", "random_ksg", "order", "treesa_original_ksg_n$(n)_s$(i).json"))
        ...
        writejson(joinpath(@__DIR__, "..", "graphs", "random_ksg", "order", "slice$(target)_original_ksg_n$(n)_s$(i).json"), sliced_code)
    end
    return nothing
end
