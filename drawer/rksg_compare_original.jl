...
begin
    ...
    ax1 = Axis(fig[1, 1], xlabel = L"\text{log}_2(\text{sc}_0)", ylabel = L"\text{log}_2(\text{tc})")
    ax2 = Axis(fig[2, 1], xlabel = L"Δ \text{log}_2(\text{sc})", ylabel = L"Δ \text{log}_2(\text{tc})")
    ...
    df_tn = [CSV.read("../data/complexity/original_ksg_n$(n).csv", DataFrame) for n in n_tn]
    df_ds = [CSV.read("../data/complexity/slice32_rksg_n$(n).csv", DataFrame) for n in n_ds]
    df_tnbb = [CSV.read("../data/complexity/tnbb_ksg_n$(n).csv", DataFrame) for n in n_tnbb]
    ...
    Legend(fig[3, :], [sc_tn, sc_tnbb, sc_ds], ["Tropical TN", "TNBB", "Dynamic Slicing"], orientation = :horizontal, nbanks = 1, labelsize = 15)
