...
begin
    ...
    # 读取数据
    df_spin = CSV.read("../../data/main/complexity/spin_glass_counting/lattice_J+-1_h_05.csv", DataFrame, missingstring=["-", ""])
    ...
    if length(n_pure_tn_vals) > 0 && fit_tn !== nothing
        ...
        lines!(ax1_top, xs_tn, model_tn(xs_tn, fit_tn.param), color = method_colors["TN"], linestyle = :dash, linewidth = 2)
    end
    ...
    ax_inset = Axis(fig1.scene, 
                    ...
                    xgridvisible = false,
                    ygridvisible = false,
                    leftspinecolor = method_colors["TN"],
                    leftspinevisible = true,
                    ytickcolor = method_colors["TN"],
                    yticklabelcolor = method_colors["TN"],
                    ylabelcolor = method_colors["TN"],
    ...
    if length(all_n_vals_inset) > 0
        ...
        if fit_tn !== nothing
            lines!(ax_inset, xs_inset, model_tn(xs_inset, fit_tn.param), color = method_colors["TN"], linestyle = :solid, linewidth = 2)
        end
    ...
    lines!(ax1_right, xs, model_tn(xs, fit_tn.param), color = method_colors["TN"], linestyle = :dash)
    ...
    sc_tn = scatter!(ax1_right, n_tn, tc_tn, markersize = t * 1.0, marker = :utriangle, color = method_colors["TN"], strokewidth = 1.0, strokecolor = :black, label = "TN")
    ...
    sc_tn_legend = scatter!(ax1_right, [NaN], [NaN], markersize = t * 1.0, marker = :utriangle, color = method_colors["TN"], strokewidth = 1.0, strokecolor = :black, label = "Tropical-TN")
