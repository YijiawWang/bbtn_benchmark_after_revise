...
df_finetune = CSV.read("../data/finetune/finetune_n60_s1.csv", DataFrame)

begin
    fig = Figure(backgroundcolor = RGBf(1.0, 1.0, 1.0), size = (600, 400), fontsize = 20)
    ax_fig = Axis(fig[1, 1], xlabel = L"N_{iters}", ylabel = L"t.c.", xticks = (0:100:500, [L"0", L"100", L"200", L"300", L"400", L"500"]), yticks = (25:5:55, [L"2^{25}", L"2^{30}", L"2^{35}", L"2^{40}", L"2^{45}", L"2^{50}", L"2^{55}"]))
