...
begin
    ...
    n_tn_large = [50:5:70...]
    df_tns_large = [CSV.read("../../data/main/complexity/random_ksg/original_ksg_n$(n).csv", DataFrame) for n in n_tn_large]
    tc_tns_large = [geometric_mean(df_tns_large[i].tc) for i in 1:length(n_tn_large)]

    df_bbs = [CSV.read("../../data/sm/count_vc/ksg_n$(n)_count_vc.csv", DataFrame) for n in n_bb]
    df_bbs_randn = [CSV.read("../../data/sm/count_vc/ksg_n$(n)_count_add_xiao2021.csv", DataFrame) for n in n_bb_randn]
    df_tns = [CSV.read("../../data/main/complexity/random_ksg/original_ksg_n$(n).csv", DataFrame) for n in n_tn]
    df_scip = [CSV.read("../../data/main/runtime/mis/ksg_n$(n)_scip_nodes.csv", DataFrame) for n in n_scip]
    ...
    sc_tn = scatter!(ax1, n_tn, tc_tns, markersize = markersize, marker = markerstyle[2], color = method_colors["TN"], strokewidth = strokewidth, strokecolor = :black, label = "Tropical TN")
    scatter!(ax1, n_tn_large, tc_tns_large, markersize = markersize, marker = markerstyle[2], color = :white, strokewidth = 2, strokecolor = method_colors["TN"])
...
begin
    ...
    df_tns = [CSV.read("../../data/main/complexity/random_ksg/original_ksg_n$(n).csv", DataFrame) for n in n_tn]
    df_bbs = [CSV.read("../../data/sm/count_vc/additional_ksg_n$(n)_count_vc.csv", DataFrame) for n in n_bb]
    df_ips = [CSV.read("../../data/main/runtime/mis/ksg_n$(n)_scip_nodes.csv", DataFrame) for n in n_ip]

    count_bbs = log2.([geometric_mean(df_bbs[i].count) for i in 1:length(n_bb)])
    count_ips = log2.([geometric_mean(df_ips[i].nodes) for i in 1:length(n_ip)])
    tc_tns = [geometric_mean(df_tns[i].tc) for i in 1:length(n_tn)]

    sc_tn = scatter!(ax0, n_tn, tc_tns, markersize = markersize, marker = markerstyle[2], color = method_colors["TN"], strokewidth = strokewidth, strokecolor = :black, label = "Tropical TN")
    ...
    lines!(ax0, xs, model_tn(xs, fit_tn.param), color = method_colors["TN"], linestyle = :dash)
