...
begin
    # 读取CSV文件并提取数组
    df = CSV.read("../data/runtime/mapped_ksg_tnbb_ds.csv", DataFrame)
    
    # 将每一行数据存储到同名数组中
    factorization = [df[1, :tnbb_branching_time], df[1, :tnbb_contract_time], df[1, :dynamic_slicing_time]]
    arbitrary_mis = [df[2, :tnbb_branching_time], df[2, :tnbb_contract_time], df[2, :dynamic_slicing_time]]
    qubo = [df[3, :tnbb_branching_time], df[3, :tnbb_contract_time], df[3, :dynamic_slicing_time]]
    
    # 检查并清理数据中的NaN值
    function clean_data(data)
        return [isnan(x) ? 0.0 : Float64(x) for x in data]
    end
    
    factorization = clean_data(factorization)
    arbitrary_mis = clean_data(arbitrary_mis)
    qubo = clean_data(qubo)
    
    # 打印数据用于调试
    println("factorization: $factorization")
    println("arbitrary_mis: $arbitrary_mis")
    println("qubo: $qubo")
    ...
    problem_data = [factorization, arbitrary_mis, qubo]
    ...
    x_grp = [1, 1, 1, 2, 2, 2, 3, 3, 3]
    bar_grp = [1, 1, 2, 1, 1, 2, 1, 1, 2]
    y_grp = [1, 2, 1, 1, 2, 1, 1, 2, 1]

    cat = [1, 2, 3, 1, 2, 3, 1, 2, 3]
    ...
    # # 手动添加图例
    Legend(fig[1, :], 
        [PolyElement(polycolor = colors[1]), PolyElement(polycolor = colors[2]), PolyElement(polycolor = colors[3])],
        ["TNBB Branching", "TNBB Contract", "Dynamic Slicing"],
        nbanks = 1,
        labelsize = 12,
    ...
    # 保存图片（PDF和PNG格式）
    save("../figs/mapped_ksg_runtime_comparison.pdf", fig)
    save("../figs/mapped_ksg_runtime_comparison.png", fig, px_per_unit = 3)
